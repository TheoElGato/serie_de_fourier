...
class Dessin:
    def __init__(self, screen, precision, figure):
        ...
        for i in range(self.precision):
            self.para.append(self.parametre(n, self.figure))
            n = n + (-1) ** i * (i + 1)

    def draw_figure(self, t):
        x, y = 0, 0
        n = 0
        for i in range(self.precision):
            oldx, oldy = x, y
            x, y = vecteur(self.screen, oldx, oldy, t, n, self.para[i])
            n = n + ((-1) ** i) * (i + 1)
        ...
        for i in range(len(self.figure)):
            point(self.screen, pygame.Vector2(self.path(i)))
    ...
    def parametre(self, n, figure):
        a = [0, 0]
        dt = 1 / len(figure)
        for i in range(len(figure)):
            t = i * dt
            tempo = self.mult_pos_tab(self.path(i), self.dummy_vecteur(t, n))
            tempo = self.mult_tab_int(tempo, dt)
            a = self.add_pos_tab(a, tempo)
        return a
